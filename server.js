...
function initDatabase() {
    db.serialize(() => {
        ...
        seedIfEmpty('team', () => {
            const teamStmt = db.prepare("INSERT INTO team (name, role, category, description, icon, skills) VALUES (?, ?, ?, ?, ?, ?)");
            const teamData = [
        ...
        seedIfEmpty('risks', () => {
            const riskStmt = db.prepare("INSERT INTO risks (title, context, mitigation, prob, impact, color, icon) VALUES (?, ?, ?, ?, ?, ?, ?)");
            const riskData = [
        ...
        // 3. Budget & BOM Tables
        db.run("CREATE TABLE IF NOT EXISTS budget_summary (category TEXT, amount INTEGER)");
        seedIfEmpty('budget_summary', () => {
            db.run("INSERT INTO budget_summary VALUES ('Total', 60000), ('CAPEX', 40000), ('OPEX', 20000)");
        });
        ...
        seedIfEmpty('gantt', () => {
            const ganttStmt = db.prepare("INSERT INTO gantt (phase, task, start_week, duration, status, color, icon) VALUES (?, ?, ?, ?, ?, ?, ?)");
            const ganttData = [
        ...
        seedIfEmpty('comms', () => {
            const commStmt = db.prepare("INSERT INTO comms (type, sender, recipient, subject, content, icon) VALUES (?, ?, ?, ?, ?, ?)");
            const commsData = [
        ...
        db.run("CREATE TABLE IF NOT EXISTS milestones (name TEXT, date TEXT, owner TEXT, status TEXT)");
        seedIfEmpty('milestones', () => {
            db.run("INSERT INTO milestones VALUES ('PCB Design Review', 'Dec 15, 2025', 'Sarah Chen', 'In Progress'), ('Dataset Collection', 'Dec 20, 2025', 'Marcus Johnson', 'In Progress'), ('Initial Prototype', 'Jan 10, 2026', 'Team', 'Pending')");
        });
    });
}
...
app.get('/api/gantt', (req, res) => {
    db.all("SELECT * FROM gantt ORDER BY phase ASC, start_week ASC", [], (err, rows) => {
        if (err) return res.status(500).json({error: err.message});
        res.json(rows);
    });
});
...
app.get('/api/dashboard', (req, res) => {
    db.all("SELECT * FROM milestones", [], (err, milestones) => {
        ...
        db.get("SELECT amount FROM budget_summary WHERE category='Total'", [], (err, total) => {
            if (err) return res.status(500).json({error: err.message});
            
            // Fetch Top 3 Risks (High Impact/Prob)
            db.all("SELECT title, color FROM risks ORDER BY impact DESC, prob DESC LIMIT 3", [], (err, risks) => {
                if (err) return res.status(500).json({error: err.message});
...
app.post('/api/simulate-risk', (req, res) => {
    ...
    if (type === 'delay') {
        ...
        db.serialize(() => {
            ...
            db.get("SELECT start_week, duration FROM gantt WHERE task LIKE '%Sourcing%'", (err, row) => {
                if (err || !row) return res.status(500).json({error: 'Task not found'});
                
                const shiftStart = row.start_week + row.duration;
                
                db.run("UPDATE gantt SET duration = duration + 2, status = 'Delayed', color = '#ef4444' WHERE task LIKE '%Sourcing%'");
                db.run("UPDATE gantt SET start_week = start_week + 2 WHERE start_week >= ?", [shiftStart]);
                res.json({ message: 'Supply Chain Delay Simulated: Timeline shifted by 2 weeks.' });
            });
        });
        ...
        const newTask = ['3. DEV', 'Sprint 4b: Heater Redesign', 13, 2, 'Critical', '#ef4444', 'fa-fire'];
        db.serialize(() => {
            // Shift tasks starting at week 13 or later by 2 weeks
            db.run("UPDATE gantt SET start_week = start_week + 2 WHERE start_week >= 13");
            // Insert new task
            db.run("INSERT INTO gantt (phase, task, start_week, duration, status, color, icon) VALUES (?, ?, ?, ?, ?, ?, ?)", newTask);
            res.json({ message: 'Sensor Fogging Issue: Added "Heater Redesign" sprint.' });
        });
    } else {
...
app.post('/api/gantt', (req, res) => {
    ...
    db.run("INSERT INTO gantt (task, phase, start_week, duration, status, color, icon) VALUES (?, ?, ?, ?, ?, ?, ?)", 
...
app.put('/api/gantt/:id', (req, res) => {
    ...
    db.run("UPDATE gantt SET task = ?, phase = ?, start_week = ?, duration = ?, status = ?, color = ? WHERE id = ?", 
